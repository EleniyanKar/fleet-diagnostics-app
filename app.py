...
if uploaded_file is not None:
    ...
    if run_clicked:
        with st.spinner("Analyzing " + str(len(df)) + " rows..."):
            ...
            fleet_sim_set = set(df["sim_number_norm"])
            airtel_found_on_server = len(airtel_numbers & fleet_sim_set)
            airtel_not_on_server = len(airtel_numbers) - airtel_found_on_server
        ...
        st.subheader("Airtel List vs Server (verified match)")
        va, vb = st.columns(2)
        va.metric("Airtel lines found on server", airtel_found_on_server)
        vb.metric("Airtel lines NOT on server", airtel_not_on_server)

        airtel_missing_from_server = sorted(airtel_numbers - fleet_sim_set)
        missing_df = pd.DataFrame({"airtel_number_not_on_server": airtel_missing_from_server})
        st.download_button(
            "Download Airtel lines NOT on server (CSV)",
            missing_df.to_csv(index=False),
            "airtel_not_on_server.csv",
            "text/csv",
        )
